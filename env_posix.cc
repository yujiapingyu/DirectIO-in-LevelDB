...
constexpr const int kDefaultMmapLimit = (sizeof(void*) >= 8) ? 0 : 0;
...
class PosixRandomAccessFile final : public RandomAccessFile {
 public:
  ...
  Status Read(uint64_t offset, size_t n, Slice* result,
    ...
    // cal the offset 
    uint64_t offset_ = offset / 4096 * 4096;
    size_t n_ = (n / 4096 + 2) * 4096;
    uint64_t z = offset % 4096;
    ...
    if (!has_permanent_fd_) {
      ...
      // fd = ::open(filename_.c_str(), O_RDONLY | O_SYNC | O_DIRECT);
      if (fd < 0) {
        return PosixError(filename_, errno);
      }
    }
    ...
    Status status;
    int r = posix_memalign((void**)&scratch, 4096, n_);
    // printf("align = %d\n", r);
    // printf("page_size:%d\n", getpagesize()); 4096bytes
    ssize_t read_size = ::pread(fd, scratch, n_, static_cast<off_t>(offset_));
    *result = Slice(scratch+z, (read_size < 0) ? 0 : n);
...
class PosixEnv : public Env {
 public:
  ...
  Status NewRandomAccessFile(const std::string& filename,
    ...
    *result = nullptr;
    printf("func(NewRandomAccessFile):open file\n");
    // int fd = ::open(filename.c_str(), O_RDONLY | O_SYNC | O_DIRECT);
